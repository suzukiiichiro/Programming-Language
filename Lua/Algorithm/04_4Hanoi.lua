...
Hanoi={};Hanoi.new=function()
  local this={
    Moves="0"
  };

  function Hanoi:Hanoi(n,from,work,dest)
    if n==1 then
      print("move "..from.."->"..dest);
    else
      self:Hanoi(n-1,from,dest,work);
      print("move "..from.."->"..dest);
      self:Hanoi(n-1,work,from,dest);
    end 
    self.Moves=self.Moves+1;
  end
  
  function Hanoi:execHanoi(n)
    --[[#第二引数から第四引数へ n 枚の円盤を移動]]--
    self:Hanoi(n,"A","B","C");
    print("Total moves = "..self.Moves);
  end
  return setmetatable(this,{__index=Hanoi});
end

s=os.clock();
print(Hanoi.new():execHanoi(3));
print(os.clock()-s);
